...
We noticed the units of `voltage` are not consistent between `sonoma-data-net.csv` and `sonoma-data-log.csv`. 
...
Similarly significant, each value have a lower frequency in the net file than the log file, indicating the convertion is successful. 

A unit conversion is also performed on variable `hamatop` and `hamabot`. 
After research, we discovered that PAR measurements are stored in the unit of Lux. 
